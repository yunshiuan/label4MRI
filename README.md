# MRI-labeling: label MNI cooridnate with AAL & BA system
- ##### Under the R program environment,input an MNI coordinate, output the corresponding AAL(Automated Anatomical Labeling) and BA (Brodmann area) brain region name.  If the coordinate does not match a brain region defined by AAL/BA (e.g., white matter), then find the closest brain region with the corresponding distance.
&#13;&#10;
